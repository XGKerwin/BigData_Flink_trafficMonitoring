 ...
 * @param policeId     警号
 * @param car          车牌
 * @param actionStatus 处理状态
 * @param actionTime   处理时间
 */
 ...
 * 2、交警实时出警记录数据
 *
 * 9999 皖M16450,套牌车辆,1592722028000
 * 8888 J0001,皖M16450,处理完成,1592722033000
 */
object ViolationCarAndPoliceActionAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val stream1: DataStream[ViolationInfo] = streamEnv.socketTextStream("node01", 9999)
      .map(line => {
    ...
    val stream2: DataStream[PoliceAction] = streamEnv.socketTextStream("node01", 8888)
      .map(line => {
        val arr: Array[String] = line.split(",")
